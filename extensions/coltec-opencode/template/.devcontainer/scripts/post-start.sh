...
# Start OpenCode in a dedicated tmux session
if command -v opencode >/dev/null 2>&1; then
  if command -v tmux >/dev/null 2>&1; then
    OPENCODE_SESSION="${OPENCODE_TMUX_SESSION:-opencode}"
    if ! tmux has-session -t "${OPENCODE_SESSION}" 2>/dev/null; then
      echo "[post-start] Starting OpenCode tmux session (${OPENCODE_SESSION})..."
      tmux new-session -d -s "${OPENCODE_SESSION}" "OPENCODE_TMUX_DISABLE=true opencode start"
      echo "[post-start] OpenCode tmux session started"
    else
      echo "[post-start] OpenCode tmux session already running"
    fi
  else
    echo "[post-start] tmux not installed; skipping OpenCode session"
  fi
else
  echo "[post-start] opencode not installed; skipping OpenCode session"
fi
